...
class MatlabInterface:
    ...
    def __init__(self, matlab_session = ''):
        ...
        if not import_fail:
            if matlab_session in matlab.engine.find_matlab():
                # link to a shared session of MATLAB
                print("MATLAB session",matlab_session,"detected, Link to this one.")
                self.eng = matlab.engine.connect_matlab(matlab_session)
            elif matlab.engine.find_matlab():
                print("No MATLAB session with input name. Link to the first detected one instead.")
                self.eng = matlab.engine.connect_matlab()
            else:
                print("Cannot find a running MATLAB session.Start a new one.")
